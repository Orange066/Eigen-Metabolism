...
if run_type == 'select' or run_type == 'mask':
...
if run_type == 'mask':
    save_path = save_path + '_' + str(sparse)
...
if run_type == 'mask':
    column_ids = positive_samples.iloc[:, 0].tolist()
    column_ids = np.array(column_ids)
    # print('column_ids', column_ids)
    # print('len(column_ids)', len(column_ids))
    # exit(0)
...
if run_type != 'mask':
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        def linear_decay(epoch, total_epochs, tau_initial=0.4, tau_final=0.0001):
            tau = tau_initial - (tau_initial - tau_final) * (epoch / total_epochs)
            return tau
        ...
        for epoch in range(epoch_all):
            ...
            for batch_X, batch_y in train_loader:
                ...
                batch_X = batch_X.cuda()
                batch_y = batch_y.cuda()
                tau = linear_decay(epoch, epoch_all, 1.0, 0.0001)
                outputs, mask = model(batch_X, tau)
                loss = criterion(outputs[:,0], batch_y) + torch.abs(torch.mean(mask) - tgt_sparse) * 50.0
                loss.backward()
                optimizer.step()
                pred_classes = (outputs > 0.5).long()
                accu_num += (pred_classes[:, 0] == batch_y.long()).sum()
        ...
        #######################################
        #######################################
        mask = best_model.get_mask()
        mask = gumbel_softmax(mask, 2, 0.0001)
        mask = (mask[:, :, 0] > mask[:, :, 1]).float()

        # X_train_aug, X_val, y_train_aug, y_va
        # 将 mask 应用于数据集
        selected_attributes = mask.cpu().squeeze() > 0.5  # 转换为 boolean 索引
        selected_columns = X_train_aug.columns[selected_attributes.tolist()]  # 获取选中的列名

        # 获取选中的索引
        selected_indices = torch.nonzero(selected_attributes).squeeze().tolist()  # 获取选中的索引

        # 将索引保存到txt文件中
        print(os.path.join(save_base, organ, save_path, 'index_' + str(count).zfill(4) + '.txt'))
        with open(os.path.join(save_base, organ, save_path, 'index_' + str(count).zfill(4) + '.txt'), 'w') as f:
            for index in selected_indices:
                f.write(f"{index}\n")
        select_names = [ column_ids[i]  for i in selected_indices]
        with open(os.path.join(save_base, organ, save_path, 'select_enzyme_' + str(count).zfill(4) + '.txt'), 'w') as f:
            for index in select_names:
                f.write(f"{index}\n")
        # 过滤数据集
        print('X_train_aug', X_train_aug.shape)
        X_train_aug = X_train_aug[selected_columns]
        X_val = X_val[selected_columns]
        X_test = X_test[selected_columns]

        print('X_train_aug_select', X_train_aug.shape)
        print('X_val_select', X_val.shape)
        print('X_test_select', X_test.shape)
        ...
        for epoch in range(epoch_all):
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                # best_model = model
                torch.save(model.state_dict(), os.path.join(save_base, organ, save_path, 'best_model_select_'+ str(count).zfill(4) + '.pth'))
        ...
        best_model = TransformerModelLarge(X_train_aug.shape[1])
        best_model.load_state_dict(
            torch.load(os.path.join(save_base, organ, save_path, 'best_model_select_' + str(count).zfill(4) + '.pth')))
        best_model = best_model.cuda()

        auc, f1, acc, precision, recall = print_classification_reports(best_model, X_val, y_val, X_test, y_test)
        acc_select_test += acc
        prec_select_test += precision
        rec_select_test += recall
        f1_select_test += f1
        if np.isnan(auc) == False:
            auc_select_test += auc
            auc_select_count += 1
        ...
        results = ['Fold_' + str(count).zfill(2), auc, f1, acc, precision, recall]
        results_select_l.append(results)
    ...
    print("Test Mean AUC Select:", auc_select_test / auc_count)
    print("Test Mean F1-Score Select:", f1_select_test / 5)
    print("Test Mean Accuracy Select:", acc_select_test / 5)
    print("Test Mean Precision Select:", prec_select_test / 5)
    print("Test Mean Recall Select:", rec_select_test / 5)

    results_select_l.append(['Average', auc_select_test / auc_select_count, f1_select_test / 5, acc_select_test / 5, prec_select_test / 5, rec_select_test / 5])

    cells = ['', 'AUC', 'F1', 'ACC', 'PREC', 'REC']
    results_select_l.insert(0, cells)
    wb = openpyxl.Workbook()
    ws = wb.active

    for row_index, data_list in enumerate(results_select_l, start=1):
        for col_index, value in enumerate(data_list, start=1):
            if isinstance(value, (float, int)):  # 仅格式化数值类型
                value = "{:.4f}".format(value)
            ws.cell(row=row_index, column=col_index, value=value)
    wb.save(os.path.join(save_base, organ, save_path, 'results_select.xlsx'))


    def read_indices_from_files(directory):
    ...
    # 定义筛选前1%属性的函数
    def filter_indices_by_percentage(index_counts, total_attributes, sparse):
        threshold_count = max(1, int(total_attributes * sparse))

        # 将索引按出现次数排序
        sorted_indices = sorted(index_counts.items(), key=lambda item: item[1], reverse=True)
        print('threshold_count', threshold_count)
        print('len(sorted_indices)', len(sorted_indices))
        if threshold_count < len(sorted_indices):
            p = sorted_indices[threshold_count]
            p_count = p[1]
        else:
            p_count = sorted_indices[-1][1]

        # 选择前 threshold_count 个属性
        selected_indices = [index for index, count in sorted_indices if count >= p_count]
    ...
    directory = os.path.join(save_base, organ, save_path)  # 替换为你的目录路径
    total_attributes = len(column_ids)  # 替换为你的属性总数量
    output_file = os.path.join(save_base, organ, save_path, 'merge_index.txt')

    index_counts = read_indices_from_files(directory)
    filtered_indices = filter_indices_by_percentage(index_counts, total_attributes, sparse)

    with open(output_file, 'w') as file:
        for index in filtered_indices:
            file.write(f"{index}\n")

    select_names = [ column_ids[i]  for i in filtered_indices]
    with open(os.path.join(save_base, organ, save_path, 'merge_enzyme.txt'), 'w') as f:
        for index in select_names:
            f.write(f"{index}\n")
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        selected_columns = X_train_aug.columns[filtered_indices]
        X_train_aug = X_train_aug[selected_columns]
        X_val = X_val[selected_columns]
        X_test = X_test[selected_columns]
        ...
        for epoch in range(epoch_all):
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                # best_model = model
                torch.save(model.state_dict(), os.path.join(save_base, organ, save_path, 'best_model_merge_'+ str(count).zfill(4) + '.pth'))
        ...
        best_model.load_state_dict(torch.load(os.path.join(save_base, organ, save_path, 'best_model_merge_'+ str(count).zfill(4) + '.pth')))
        best_model = best_model.cuda()
    ...
    for row_index, data_list in enumerate(results_l, start=1):
        for col_index, value in enumerate(data_list, start=1):
            if isinstance(value, (float, int)):  # 仅格式化数值类型
                value = "{:.4f}".format(value)
            ws.cell(row=row_index, column=col_index, value=value)
    wb.save(os.path.join(save_base, organ, save_path, 'results_merge.xlsx'))